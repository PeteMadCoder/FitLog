...
class FakePermissionService implements PermissionService {
  ...
  @override
  Future<bool> hasBackgroundLocationPermission() async => backgroundLocationPermission;

  @override
  Future<bool> requestBackgroundLocationPermission() async => backgroundLocationPermission;
...
void main() {
  group('TrackingNotifier Tests', () {
    ...
    test('timer increments duration while recording', () async {
      ...
      expect(container.read(trackingNotifierProvider).durationSeconds, equals(0.0));

      notifier.tick();
      notifier.tick();
      notifier.tick();

      expect(container.read(trackingNotifierProvider).durationSeconds, equals(3.0));
    });
    ...
    test('pauseTracking pauses timer and recording state', () async {
      ...
      notifier.tick();
      notifier.tick();
      expect(container.read(trackingNotifierProvider).durationSeconds, equals(2.0));

      notifier.pauseTracking();
      expect(container.read(trackingNotifierProvider).status, equals(TrackingStatus.paused));
    ...
    test('resumeTracking resumes recording and timer', () async {
      ...
      expect(container.read(trackingNotifierProvider).status, equals(TrackingStatus.recording));
    ...
    test('stopTracking cleans up, returns state and resets notifier', () async {
      final notifier = container.read(trackingNotifierProvider.notifier);
      await notifier.startTracking('running');
      ...
      final recordedState = notifier.stopTracking();

      expect(recordedState.status, equals(TrackingStatus.recording));
      expect(recordedState.durationSeconds, equals(5.0));
